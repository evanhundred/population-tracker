...
class MainContent {
  constructor(ele) {
    ...
    this.createInstructionsLink();
    this.createVintageSelector();
    this.createDataElements();

    new Legend(this.ele);
    this.updateData('2020');
    ...
    document.addEventListener('click', function (e) {
      ...
      if (eventTarget.id === 'vintage') {
        ...
        const sortedDataObj = sortData(this.dataObject, sortStyle);
        this.printer.sortByName(sortedDataObj, sortStyle);
      }
    }.bind(this));
  }

  updateData(vintage) {
    const firstLineFooterH2 = document.getElementById('firstLineFooterH2');
    firstLineFooterH2.innerText = 'fetching...';

    this.fetcher.fetchData(vintage).then(dataObject => {
      this.dataObject = dataObject;
      this.renderData(this.dataObject);
  ...
  renderData(dataObject) {
    this.resetMap();
    new Map(dataObject);
    this.printer.printData(dataObject, this.sortStyle);
    this.printer.sortByName(dataObject, this.sortStyle);
  }
  ...
  createInstructionsLink() {
    ...
    const instructionsModalLink = document.createElement('h3');
    instructionsModalLink.innerText = 'View instructions';
    instructionsModalLink.addEventListener('click', () => {
      const instructionsModalContainer = document.createElement('div');
      instructionsModalContainer.setAttribute('id', 'instructions-modal-container');
      mainNode.appendChild(instructionsModalContainer);
      new instructionsModal(instructionsModalContainer);
